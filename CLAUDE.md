...
Schlanke macOS-Menüleisten-App, die an bevorstehende Kalender-Events erinnert und das Beitreten zu Online-Meetings per Klick ermöglicht. Unterstützt 8 Meeting-Provider (Teams, Zoom, Google Meet, WebEx, GoTo, Slack, Whereby, Jitsi). Vollbild-Overlay mit konfigurierbarer Vorlaufzeit, Screen-Sharing-Schutz, Sleep/Wake-Handling und globalem Keyboard-Shortcut. Maximal ressourcenschonend (kein Polling, event-basiertes Scheduling).

**Dokumentation:** `docs/superpowers/specs/2026-03-20-meeting-reminder-design.md` (Design Spec)
...
**14 Swift-Dateien | 4 Test-Dateien | 153 Tests**
...
xcodegen generate
xcodebuild build -project "Meeting Reminder.xcodeproj" -scheme "Meeting Reminder" \
...
```bash
xcodebuild test -project "Meeting Reminder.xcodeproj" -scheme "Meeting Reminder" \
  -destination "platform=macOS" -derivedDataPath ./build \
  CODE_SIGN_IDENTITY="Apple Development: Hendrik Grueger (HY44A7L7D7)" DEVELOPMENT_TEAM=CU87QNNB3N
```
